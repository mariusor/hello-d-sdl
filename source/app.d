...
GLuint compileShaders(string vertexSrc, string fragmentSrc)
{
	...
	if (infoLogLength > 0) {
		...
		if (infoLog.length > 0) {
			writeln("vertex shader info log: " ~ infoLog);
		}
	}
...
//	}
	...
	if (infoLogLength > 0) {
		...
		writeln("fragment shader info log: " ~ infoLog);
	}
...
//	}
	...
	if (infoLogLength > 0) {
		...
		writeln("linking info log: " ~ infoLog);
	}
...
bool frame(SDL_Window* window, GLuint vertexBufferID, GLuint programID, Events *ev)
{
...
void main()
{
	...
	while(!ev.quitting) {
		if(!frame(window, vertexBufferID, programID, &ev)) { break; }
	}
